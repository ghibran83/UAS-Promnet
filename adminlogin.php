...
// Kalo udah masuk, langsung dashboard aja
...
// Kalo ada yang kirim data login (POST)
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Tangkap isian form
    $username = trim($_POST['username'] ?? '');
    $password = $_POST['password'] ?? '';
    
    // Cek isiannya kosong ga
    if (empty($username) || empty($password)) {
        $pesan = 'Username sama password ga boleh kosong bos!';
        $tipe_pesan = 'danger';
    } else {
        // Coba loginin
        $hasil = loginAdmin($username, $password);
        
        if ($hasil['status']) {
            // Simpen sesi biar awet
            $_SESSION['admin_id'] = $hasil['data']['id_admin'];
            $_SESSION['admin_nama'] = $hasil['data']['nama'];
            $_SESSION['admin_username'] = $hasil['data']['username'];
...
<head>
    <meta charset="utf-8">
    <title>Sistem Peminjaman - Login Admin</title>
                    ...
                    <div class="bg-secondary rounded p-4 p-sm-5 my-4 mx-3">
                        ...
                        <form method="POST" action="">
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput" name="username" placeholder="Username" value="<?= htmlspecialchars($username ?? '') ?>" required>
                            ...
                            <p class="text-center mb-0">Belum punya akun? <a href="adminregist.php">Daftar di sini</a></p>
